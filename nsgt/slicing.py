...
def makewnd(sl_len, tr_area):
    ...
    tw = torch.empty(sl_len, dtype=torch.float32, device=get_torch_device())
    tw[:hhop-htr] = 0
...
def slicing(f, sl_len, tr_area):
    ...
    fseq = reblock(f, hhop, dtype=torch.float32, fulllast=True, padding=0., multichannel=True)
    
    # get first block to deduce number of channels
    fseq0 = next(fseq)
    chns = len(fseq0)
    pad = torch.zeros((chns,hhop), dtype=fseq0.dtype, device=get_torch_device())
    ...
    for fi in fseq:
        past.append(fi)
        if len(past) == 4:
            f_slice = torch.empty((chns,sl_len), dtype=fi.dtype, device=get_torch_device())
            sl = next(slices)
